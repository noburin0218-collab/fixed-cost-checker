...
NEG = r"(ありません|ではない|ではありません|とまでは言えません|言えません|誤りです|ものではない|扱わず|扱いません|示していません|作りません|付けません|一択、ではありません|とは限りません|とは限らない|とはなりません|とは判断しません|判断しません)"
...
# ---- 2c-2) 第7バッチのYMYL：公的保障・車 ----
# 公的保障（厚労省・日本年金機構／2026-09-03 確認）
a7=open('articles/kouteki-hoshou-kakunin/index.html',encoding='utf-8').read()
for must in ["2026年8月","年間上限","多数回該当","通算1年6か月","847,300円","243,800円","81,300円",
             "1,091,100円","1,334,900円","報酬比例部分","2027年8月"]:
    if must not in a7: fails.append(f"[公的保障 一次情報] '{must}' の記載が無い")
# 制度の断定・一律化をしていないか
assert_not_asserted(a7, ["民間保険はいらない","公的保障だけで十分","誰でも9.3万円",
                          "9.3万円あれば十分","必ず受け取れる","誰にでも同じ傷病手当金"],
                    "公的保障 断定", fails)
# 「例であって全員の金額ではない」旨と、確認先への誘導があるか
for need, label in [("特定の条件での例", "9.3万円が例であることの明示"),
                    ("保険者", "健康保険の保険者への確認導線"),
                    ("日本年金機構", "年金の確認先")]:
    if need not in a7: fails.append(f"[公的保障] {label}が無い")

# 車（国税庁・国交省・損保料率機構／2026-09-03 確認）
a8=open('articles/kuruma-hiyou-bunkai/index.html',encoding='utf-8').read()
for must in ["自動車重量税","次回自動車重量税額照会サービス","自動車検査証の有効期間",
             "17,650円","17,540円","18,560円","18,660円","2026年11月1日","保険期間の始期"]:
    if must not in a8: fails.append(f"[車 一次情報] '{must}' の記載が無い")
# 自賠責の金額は、条件（地域・契約期間）と同じ視界に置いているか
m_car = re.search(r'<caption>自賠責保険の基準料率の例[^<]*</caption>', a8)
if not m_car:
    fails.append("[車] 自賠責の表に条件付きのcaptionが無い")
else:
    cap = m_car.group(0)
    for need in ["離島以外", "沖縄県を除く", "24か月"]:
        if need not in cap: fails.append(f"[車] 自賠責の条件 '{need}' が表題に無い")
# 税額・車検周期の一律化、保有の是非の断定をしていないか
assert_not_asserted(a8, ["すべての車が2年ごと","すべての車は2年ごと","普通車の重量税は一律",
                          "維持費の平均は","車を手放すべき","車は持たないほうが"],
                    "車 断定", fails)
if "用途・車種等によって異なります" not in a8:
    fails.append("[車] 車検の有効期間が用途・車種で異なる旨の記載が無い")
